...
public class GameManager : MonoBehaviour, IReceiveMessages
{
    ...
    private LobbyRelaySample.vivox.VivoxSetup m_vivoxSetup = new LobbyRelaySample.vivox.VivoxSetup();
    [SerializeField]
    private List<LobbyRelaySample.vivox.VivoxUserHandler> m_vivoxUserHandlers;
...
#pragma warning restore IDE0059
        ...
        Locator.Get.Provide(new LobbyRelaySample.Auth.Identity(OnAuthSignIn));
    ...
    {
        Debug.Log("Signed in.");
        m_localUser.ID = Locator.Get.Identity.GetSubIdentity(LobbyRelaySample.Auth.IIdentityType.Auth).GetContent("id");
    ...
    public void OnReceiveMessage(MessageType type, object msg)
    {
        if (type == MessageType.CreateLobbyRequest)
        {
            ...
            {
                LobbyRelaySample.lobby.ToLocalLobby.Convert(r, m_localLobby);
                OnCreatedLobby();
            },
                OnFailedJoin);
        }
        ...
        else if (type == MessageType.QueryLobbies)
        {
            ...
            LobbyAsyncRequests.Instance.RetrieveLobbyListAsync(
                qr => {
                    if (qr != null)
                        OnLobbiesQueried(LobbyRelaySample.lobby.ToLocalLobby.Convert(qr));
                },
                er => {
                    OnLobbyQueryFailed();
                },
