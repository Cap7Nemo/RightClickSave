...
public class Networked3rdPersonCharacter : NetworkCharacterBase
{
		...
		public Networked3rdPersonInput userControl; // user input
		...
		void FixedUpdate()
		{
			...
			// Friction
			if (userControl.localState.move == Vector3.zero && groundDistance < airborneThreshold * 0.5f) HighFriction();
			else ZeroFriction();

			bool stopSlide = !fullRootMotion && onGround && userControl.localState.move == Vector3.zero && r.velocity.magnitude < 0.5f && groundDistance < airborneThreshold * 0.5f;
			...
			{
				if (!userControl.localState.jump) jumpReleased = true;

				//r.AddForce(gravity * gravityMultiplier);
				if (jumpReleased && userControl.localState.jump && !doubleJumped && doubleJumpEnabled)
				{
					jumpEndTime = Time.time + 0.1f;
					animState.doubleJump = true;

					Vector3 jumpVelocity = userControl.localState.move * airSpeed;
			...
			ScaleCapsule(userControl.localState.crouch ? crouchCapsuleScaleMlp : 1f);
		...
		protected virtual void Update()
		{
			...
			animState.crouch = userControl.localState.crouch;
		...
		private void MoveFixed(Vector3 deltaPosition)
		{
			...
			if (!fullRootMotion)
			{
				...
				else
				{
					...
					Vector3 airMove = V3Tools.ExtractHorizontal(userControl.localState.move * airSpeed, gravity, 1f);
		...
		private bool CanWallRun()
		{
			...
			if (userControl.localState.move.magnitude < wallRunMinMoveMag) return false;
			return true;
		}
		...
		private Vector3 GetMoveDirection()
		{
			switch (moveMode)
			{
				case MoveMode.Directional:
					moveDirection = Vector3.SmoothDamp(moveDirection, new Vector3(0f, 0f, userControl.localState.move.magnitude), ref moveDirectionVelocity, smoothAccelerationTime);
					moveDirection = Vector3.MoveTowards(moveDirection, new Vector3(0f, 0f, userControl.localState.move.magnitude), Time.deltaTime * linearAccelerationSpeed);
					return moveDirection * forwardMlp;
				case MoveMode.Strafe:
					moveDirection = Vector3.SmoothDamp(moveDirection, userControl.localState.move, ref moveDirectionVelocity, smoothAccelerationTime);
					moveDirection = Vector3.MoveTowards(moveDirection, userControl.localState.move, Time.deltaTime * linearAccelerationSpeed);
		...
		protected virtual void Rotate()
		{
			...
			if (userControl.localState.move == Vector3.zero) angle *= (1.01f - (Mathf.Abs(angle) / 180f)) * stationaryTurnSpeedMlp;
		...
		{
			bool isMoving = userControl.localState.move != Vector3.zero;

			switch (moveMode)
			{
				case MoveMode.Directional:
					if (isMoving) return userControl.localState.move;
					return lookInCameraDirection ? userControl.localState.lookPos - r.position : transform.forward;
				case MoveMode.Strafe:
					if (isMoving) return userControl.localState.lookPos - r.position;
					return lookInCameraDirection ? userControl.localState.lookPos - r.position : transform.forward;
			}
		...
		protected virtual bool Jump()
		{
			...
			if (!userControl.localState.jump) return false;
			if (userControl.localState.crouch) return false;
